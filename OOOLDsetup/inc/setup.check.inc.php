...
if(!empty($step)) {
	...
	if ($step == 1 && $do) {
		...
		if(isset($_POST["dbsavesubmit"])) {
			...
			if($phpwcms["db_pers"] == 1) {
				...
				$db = @mysqli_connect($phpwcms["db_host"], $phpwcms["db_user"], $phpwcms["db_pass"]);
			}

			if(@mysqli_select_db($db, $phpwcms["db_table"])) {;
					...
					if(isset($_POST['db_sql_hidden'])) {
						...
						if(empty($db_sql)) {
                            ...
                            */
                            ...
                            $sql_data    = read_textfile('default_sql/bancoinstalador.sql');
                            //$sql_data    = preg_replace("/(#|--).*.\n/", '', $sql_data );
                            $sql_data    = preg_replace('/ `phpwcms/', ' `'.$_db_prepend.'phpwcms', $sql_data );
                            $sql_data    = str_replace("\r", '', $sql_data);
                            $sql_data    = str_replace("\n\n", "\n", $sql_data);
							...
							if(isset($_POST['db_create'])) {
                                ...
                                // $db_create_sql = explode("\n\n", $sql_data);
                                $db_commit     = explode('COMMIT;',      $sql_data);
                                $db_create     = explode('ALTER TABLE',  $db_commit[0]);
                                $db_create_sql = explode("CREATE TABLE", $db_create[0]);
                                unset($db_create_sql[0]);
                                unset($db_create[0]);
                                unset($db_commit[0]);
                                ...
                                foreach($db_create_sql as $key => $value) {
                                    ...
                                    $partes = explode('INSERT INTO', $value);

                                    //var_dump($alter);

                                    foreach($partes as $s_key => $s_value ){

                                        if($s_key == 1){
                                            $s_value = 'INSERT INTO'.utf8_decode($s_value);
                                        }

                                        if(!mysqli_query($db, $s_value)) {
                                            var_dump($s_value);
                                            echo mysqli_error($db);
                                            echo '<br><br>';
                                            $db_create_err[] = $s_value;
                                            unset($db_create_sql[$key]);
                                        }
                                ...
                                foreach($db_create as $key => $value) {
	...
	if ($step == 2 && $do) {
		...
		if(!empty($_POST["admin_create"])) {
			$db = mysqli_connect($phpwcms["db_host"],$phpwcms["db_user"],$phpwcms["db_pass"]);
			mysqli_select_db($phpwcms["db_table"],$db);
			mysqli_query($db, "SET NAMES '".$phpwcms["charset"]."'");
