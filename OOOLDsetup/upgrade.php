  ...
  <tr bgcolor="#FFFFFF">
    ...
    <td valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="0" cellspacing="0" style="border:1px dotted #7599BB;" summary="">
        ...
        <tr>
          ...
          <td class="chatlist"><?php echo mysqli_get_client_info() ?> (is just client information, server might be different)</td>
        </tr>
        <tr>
...
if($do) {
	...
	foreach($sql as $key => $value) {
		...
		if(!$value) {
			...
			if(!mysqli_query($db, $value)) echo '<span class="error">ERROR: '.html_entities(@mysqli_error($db))." -&gt; </span>";
