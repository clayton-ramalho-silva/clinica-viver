...
class GoogleMapAPI {
	 ...
	 */
	var $html5 = false;
	var $_jstag_attribute = ' type="text/javascript"';
	var $_meta_close = ' /';
	...
	var $lookup_server = array(
		'GOOGLE' => 'http://maps.google.com',
		'YAHOO' => 'http://api.local.yahoo.com'
	);
	...
	var $_db_cache_table_created = false;
	...
	/**
	 * use jQuery or Mootools to select map element
	 *
	 * @var bool
	 */
	var $_jslib_selector = "document.getElementById('%s')";
	...
	function jslibSelectorJquery()
	{
		$this->_jslib_selector = "$('#%s')";
	}

	function jslibSelectorMootools()
	{
		$this->_jslib_selector = "$('%s')";
	}

	function jslibSelectorCustom($selector="document.getElementById('%s')")
	{
		$this->_jslib_selector = $selector;
	}

	function jslibSelectorDefault()
	{
		$this->_jslib_selector = "document.getElementById('%s')";
	}

	function enableMooTools()
	{
		$this->jslibSelectorMootools();
	}
	...
	{
		return sprintf($this->_jslib_selector ? $this->_jslib_selector : "document.getElementById('%s')", $element);
	}
 ...
 function addWaypoints($lat, $lon, $stopover = TRUE)
 {
  ...
  function addWaypointByAddress($address,$stopover = TRUE)
  {
	...
	function getIconKey($iconImage,$iconShadow=""){
		return md5($iconImage.$iconShadow);
	}
	...
	function addIcon($iconImage,$iconShadowImage = '',$iconAnchorX = 'x',$iconAnchorY = 'x',$infoWindowAnchorX = 'x',$infoWindowAnchorY = 'x') {
		...
		if($result) {
			...
			$this->_marker_icons[$_iconKey] = $result;
			return $_iconKey;
		}
	}
	...
	function getHeaderJS() {
		$_headerJS = array();
		if( $this->mobile == true){
			$_headerJS[] = '<meta name="viewport" content="'.$this->meta_viewport.'"' . $this->_meta_close . '>';
		}
		if(!empty($this->_elevation_polylines)||(!empty($this->_directions)&&$this->elevation_directions)){
			$_headerJS[] = '<script' . $this->_jstag_attribute . ' src="http://www.google.com/jsapi"></script>';
			$_headerJS[] = '<script' . $this->_jstag_attribute . '>';
			$_headerJS[] = '	// Load the Visualization API and the piechart package.';
			$_headerJS[] = '	google.load("visualization", "1", {packages: ["columnchart"]});';
			$_headerJS[] = '</script>';
		}
		$scriptUrl  = "http://maps.google.com/maps/api/js?sensor=";
		$scriptUrl .= $this->mobile==TRUE ? "true" : "false";
		if( is_array( $this->api_options ) ) {
			foreach( $this->api_options as $key => $value ){
				$scriptUrl .= '&'.$key.'='.$value;
			}
		}
		$_headerJS[] = '<script' . $this->_jstag_attribute . ' src="' . $scriptUrl . '"></script>';
		if($this->marker_clusterer){
			$_headerJS[] = '<script' . $this->_jstag_attribute . ' src="' . $this->marker_clusterer_location . '"></script>';
		}
		/*TODO:
	...
	*/
	 ...
	 */
	function getOnLoad() {
		return '<script' . $this->_jstag_attribute . '> window.onload=' . $this->getOnLoadFunction() . '; </script>';
	}
	...
	function getMapJS() {
		$_script = "";
		$_key = $this->map_id;
		$_output = '<script' . $this->_jstag_attribute .' charset="utf-8">' . "\n";
		$_output .= '//<![CDATA[' . "\n";
		$_output .= "/*************************************************\n";
		$_output .= " * Created with GoogleMapAPI" . $this->_version . " for phpwcms\n";
		...
		*/
		...
		if($this->display_map){
			$_script .= 'var mapObj' . $_key . ' = ' . $this->getElementById($this->element_id) . ';' . "\n";
	...
	function getMap() {
		...
		if(strlen($this->width) > 0 && strlen($this->height) > 0) {
			$_output .= sprintf('document.write(\'<div id="%s" style="width: %s; height: %s; position:relative;"><\/div>\');',$this->element_id,$this->width,$this->height) . "\n";
		} else {
			$_output .= sprintf('document.write(\'<div id="%s" style="position:relative;"><\/div>\');',$this->element_id) . "\n";
		}
		//$_output .= '}';
	...
	function getCache($address) {
		...
		$_ret = array();
		$_row = _dbGet($this->_db_cache_table, 'lon,lat', 'address='._dbEscape($address));
		...
		if ( isset( $_row[0] ) ) {
		...
		} elseif( $_row === false && $this->_db_cache_table && !$this->_db_cache_table_created) {
			...
			if($result) {
				$this->_db_cache_table_created = true;
			}
		...
		switch($this->lookup_service) {
			case 'GOOGLE':
				...
				if($_result) {
					...
					$this->Geocode_Placemark = $_result_parts->results;
				}
				break;
	...
	*/
	...
	{
		//return acos( ( sin( deg2rad($lat1) ) * sin( deg2rad($lat2) ) ) + ( cos( deg2rad($lat1) ) * cos( deg2rad($lat2) ) * cos( deg2rad($lon1 - $lon2) ) ) ) * 6371.0; //$earthRadius;
		return $this->geoGetDistance($lat1,$lon1,$lat2,$lon2, 'K');
	}
