...
<!-- CREATE GROUP -->
		  ...
		  ?>
			<br /><table width="<?php echo $phpwcms['LarguraInterna']; ?>" border="0" cellpadding="0" cellspacing="0" summary="">
		  ...
		  <?php
			...
			if($group["id"]) {	
				...
				$sql = "SELECT * FROM ".DB_PREPEND."phpwcms_usergroup WHERE group_id=".$group["id"]." LIMIT 1";
		       	...
		       	if($result = mysqli_query($sql, $db) or die("error while getting group infos")) {
...
if($result = mysqli_query($sql, $db) or die("error while listing frontend users")) {
	$_temp_usr = array();
