		...
		if(is_array($pagelayout["layout_customblocks"]) && count($pagelayout["layout_customblocks"])) {
			...
			if(is_array($pagelayout["layout_customblocks"]) && count($pagelayout["layout_customblocks"])) {
				foreach($pagelayout["layout_customblocks"] as $key => $value) {
					...
					if(in_array($value, array('CONTENT', 'LEFT', 'RIGHT', 'HEADER', 'FOOTER', 'BANNER', 'SERVICOS', 'PRODUTOS', 'MAPA', 'CLIENTES', 'CPSET', 'SYSTEM'))) {
...
?><form action="phpwcms.php?do=admin&p=8&s=<?php echo $pagelayout["id"] ?>" method="post" name="pagelayout" target="_self"><table width="<?php echo $phpwcms['LarguraInterna']; ?>" border="0" cellpadding="0" cellspacing="0" summary="">
