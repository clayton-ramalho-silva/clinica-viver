...
<div class="botoes-newsletter">
    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;s=0&amp;edit=1">
        <i class="fas fa-envelope-open-text"></i>
<?php echo $BL['be_cnt_new_recipient'] ?>
    </a>

    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;duplicate=remove" onclick="return confirm('Excluir e-mails duplicados?');">
        <i class="far fa-clone"></i>
<?php echo $BL['be_cnt_delete_duplicates'] ?>
    </a>

<!--    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;import=1">
        <i class="fas fa-file-import"></i>
<?php echo $BL['be_newsletter_newimport'] ?>
    </a>-->

    <a class="botoes" href="include/inc_act/act_export.php?action=exportsubscriber" target="_blank" onclick="return confirm('Exportar todos os e-mails selecionados?');">
        <i class="fas fa-file-export"></i>
<?php echo $BL['be_cnt_export_selection'] ?>
    </a>
</div>
...
?>
<form action="phpwcms.php?do=messages&amp;p=4" method="post" name="paginate" id="paginate">
    <input type="hidden" name="do_pagination" value="1" />


    <table width="100%" border="0" cellpadding="0" cellspacing="0" class="dados-sumario" summary="">
        <tr>
            <td>
                <div class="grid-4 aic" style="display:none">
                <p>
                    <label for="showactive" class="botoes">
                        <input type="checkbox" name="showactive" id="showactive" value="1" onclick="this.form.submit();"<?php is_checked(1, $_userInfo['list_active'], 1) ?> />
                        Mostrar E-mails Ativos
                    </label>
                </p>

                <p>
                    <label for="showinactive" class="botoes">
                        <input type="checkbox" name="showinactive" id="showinactive" value="1" onclick="this.form.submit();"<?php is_checked(1, $_userInfo['list_inactive'], 1) ?> />
                        Mostrar E-mails Inativos
                    </label>
                </p>
                </div>
    ...
    <!--<td><img src="img/button/aktiv_12x13_1.gif" alt="" style="margin:1px 1px 0 1px;" /></td>-->
    <!--<td><img src="img/button/aktiv_12x13_0.gif" alt="" style="margin:1px 1px 0 1px;" /></td>-->
...
//	echo '<td class="chatlist">|&nbsp;</td>';
    ...
    echo '';
    echo '<input type="text" name="page" id="page" maxlength="4" size="4" value="' . $_SESSION['subscriber_page'];
    echo '"  class="textinput" style="margin:0 3px 0 5px;width:30px;font-weight:bold;" />';
    echo '/' . $_userInfo['pages_total'] . '&nbsp;';
    echo '';
    if ($_SESSION['subscriber_page'] < $_userInfo['pages_total']) {
        echo '<a href="phpwcms.php?do=messages&amp;p=4&amp;page=' . ($_SESSION['subscriber_page'] + 1) . '">';
...
?>	
                            ...
                            <p><b>Buscar E-mail</b><input type="text" name="filter" id="filter" size="10" value="<?php
                            if (isset($_SESSION['filter_subscriber']) && count($_SESSION['filter_subscriber'])) {
                                echo html(implode(' ', $_SESSION['filter_subscriber']));
                            }
                            ?>" class="textinput" title="filter results by username, name or email" /></p>
                            <!--<td><input type="image" name="gofilter" src="img/famfamfam/action_go.gif" style="margin-right:3px;" /></td>-->

   
                            <div class="barra"></div>
                            
                <div class="botoes-newsletter fl">
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=10">10</a>
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=25">25</a>
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=50">50</a>
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=100">100</a>
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=250">250</a>
                    <a class="botoes" href="phpwcms.php?do=messages&amp;p=4&amp;c=all"><?php echo $BL['be_ftptakeover_all'] ?></a>
                </div>
            </td>
...
// loop listing available newsletters
    ...
    foreach ($data as $row) {
        ...
        echo '<p class="lista-email-news"' . ( ($row_count % 2) ? ' style="background:#F3F5F8"' : '' ) . $_userInfo['channel_select'] . ">\n";
        echo '<span><i class="far fa-address-card"></i>' . "\n";
        echo '' . $row["address_email"] . "\n";
        echo '' . html($row["address_name"]) . "\n";
        echo '</span>';
//        echo '<td align="right" nowrap="nowrap" class="button_td">';
        echo '<span class="botoes-controle-paginas">';

        echo '<a class="botoes bt-editar" href="phpwcms.php?do=messages&amp;p=4&amp;s=' . $row["address_id"] . '&amp;edit=1">';
       ...
       // echo '<img src="img/button/aktiv_12x13_' . $row["address_verified"] . '.gif" border="0" alt=""></a>';
        ...
        echo '<a class="botoes bt-delete" href="phpwcms.php?do=messages&amp;p=4&amp;s=' . $row["address_id"] . '&amp;del=' . $row["address_id"];
        echo '" title="delete: ' . $row["address_email"] . '"';
        echo ' onclick="return confirm(\'Excluir email ' . js_singlequote($row["address_email"]) . '\');">';
