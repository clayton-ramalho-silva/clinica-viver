...
<table width="<?php echo $phpwcms['LarguraInterna']; ?>" border="0" cellpadding="0" cellspacing="0" summary="">
		  ...
		  ?>
		  <tr><td colspan="2"><img src="img/lines/l538_70.gif" alt="" width="<?php echo $phpwcms['LarguraInterna']; ?>" height="1" /></td>
    </tr>
		  ...
		  <?php
			...
			if($fcat["id"]) {
				...
				if($result = mysqli_query($db, $sql) or die("error while getting file category infos")) {
			...
			if(isset($_POST["fcat_aktion"]) && intval($_POST["fcat_aktion"])) { //Formular zum Bearbeiten der Dateikategorie-Namen
				...
				if(isEmpty($fcat["name"])) {
					...
					if($result = mysqli_query($db, $sql) or die("error while inserting/updating file category")) {
						if(!$fcat["id"]) $fcat["id"] = mysqli_insert_id($db);
...
<?php
		  ...
		  <?php
			...
			if($fkey["id"]) {
				...
				if($result = mysqli_query($db, $sql) or die("error while getting file key infos")) {
					if($row = mysqli_fetch_array($result)) {
			...
			if(!empty($_POST["fkey_aktion"])) { //Formular zum Bearbeiten der Dateischlüssel-Namen
				...
				if(isEmpty($fkey["name"])) {
					...
					if($result = mysqli_query($db, $sql) or die("error while inserting/updating file key")) {
						if(!$fkey["id"]) $fkey["id"] = mysqli_insert_id($db);
						headerRedirect(PHPWCMS_URL."phpwcms.php?do=admin&p=7");
					}
				}
		  ...
		  <tr>
			  ...
			  <?php
				...
				if($result = mysqli_query($db, $sql) or die("error while creating file category list")) {
...
<?php
		  ...
		  <?php
			...
			if($result = mysqli_query($db, $sql) or die("error while browsing file categories")) {
				while($row = mysqli_fetch_array($result)) {
					...
					echo "<img src=\"img/button/add_13x13.gif\" width=\"13\" height=\"13\" border=\"0\"></a>";

					echo "<a href=\"phpwcms.php?do=admin&p=7&fcatid=".$row["fcat_id"]."\" title =\"".$BL['be_admin_fcat_editcat']."\">";
					echo "<img src=\"img/button/edit_22x11.gif\" width=\"32\" height=\"13\" border=\"0\"></a>";
					...
					echo "<img src=\"img/button/del_13x13_1.gif\" width=\"13\" height=\"13\" border=\"0\"></a>";
					...
					if(isset($_SESSION["fcatlist"]) && isset($_SESSION["fcatlist"][$row["fcat_id"]]) && $_SESSION["fcatlist"][$row["fcat_id"]]) { //List key names for this categroy
						...
						if($kresult = mysqli_query($db, $ksql)) {
