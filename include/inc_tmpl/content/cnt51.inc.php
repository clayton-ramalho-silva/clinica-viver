...
if($map_selected) {
	...
	if(isset($_GET['dellocid']) && intval($_GET['dellocid'])) {
		...
		mysqli_query($db, "UPDATE ".DB_PREPEND."phpwcms_map SET map_deleted=9 WHERE ".
					"map_cid=".intval($content["id"])." AND map_id=".intval($_GET['dellocid']));
	...
	$map_xy = array();
	$ck = 0;
	if($map_result = mysqli_query($db, $map_sql)) {
		while($map_row = mysqli_fetch_assoc($map_result)) {
			...
			$map_list .= '<img src="img/button/edit_11x11.gif" width="32" height="13" border="0" alt=""></a>';
			...
			$map_list .= '<img src="img/button/del_13x13_1.gif" width="13" height="13" alt="" border="0">';
			$map_list .= "</a></td>\n</tr>\n";
...
<tr>
  ...
  <td><input name="cmap_location_title" type="text" class="f11" style="width: 584px" value="<?php echo  empty($content['location']["title"]) ? '' : html($content['location']["title"]) ?>" size="40" onChange="doMapChange();"></td>
</tr>
...
<tr>
  ...
  <td valign="top"><textarea name="cmap_text" cols="40" rows="8" class="f11" style="width:584px"><?php echo  empty($content["map"]["text"]) ? '' : html($content["map"]["text"]) ?></textarea></td>
</tr>
