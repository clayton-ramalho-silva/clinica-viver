...
if(isset($cinfo_image[2]) && is_array($cinfo_image) && count($cinfo_image)) {
	...
	if($thumb_image != false) {
		$cinfo_image = '<span class="lista-texto-imagem-img"><img src="'.PHPWCMS_IMAGES . $thumb_image[0] .'" border="0" '.$thumb_image[3].'></span>';
	} else {
		$cinfo_image = '';
	}
} else {
...
foreach($cinfo as $value) {
	if($value) $cinfo["result"] .= "<li>" . $value ."</li>";
}

//$cinfo["result"] = str_replace("\n", "/ <br> ", html(trim($cinfo["result"])));

if($cinfo["result"] || $cinfo_image) { //Zeige Inhaltinfo
	//echo "<tr><td>&nbsp;</td><td class=\"v10\">";
	echo "<a class=\"lista-texto-imagem\"  href=\"phpwcms.php?do=articles&amp;p=2&amp;s=1&amp;aktion=2&amp;id=".$article["article_id"]."&amp;acid=".$row["acontent_id"]."\">";
	echo $cinfo_image;
	echo "<ul class=\"lista-texto-imagem-txt\">" . $cinfo["result"] . "</ul>";
