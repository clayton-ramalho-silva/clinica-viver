...
if(isset($_POST["file_aktion"]) && intval($_POST["file_aktion"]) == 2) {
	...
	if(empty($file_name)) {
		...
		//Wenn Dateiname keine Erweiterung hat, dann Extension anh�ngen
		...
		if($row = mysql_fetch_array($result)) {
			...
			if(isset($row["f_hash"])) {
				...
				if($thumb_image != false) {
					$file_thumb_small = '<img src="'.PHPWCMS_IMAGES . $thumb_image[0] .'" '.$thumb_image[3].' alt="" />';
				}
			}
...
if($ja) {
?>
<form action="phpwcms.php?do=files&f=0" method="post" name="editfileinfo" id="editfileinfo" class="dados-sumario">

    <h3><?php echo $BL['be_fprivedit_title'] ?></h3>
    ...
    <?php
        ...
        if($file_thumb_small) {
            echo $file_thumb_small;
                    ...
                    echo '<img src="img/icons/small_'.extimg($file_ext).'" border="0" alt="" /> ';
                    echo html($file_name);
		}
    ...
    <p>
        ...
        <select name="file_pid" id="file_pid" class="width400">
	...
	?>
    <p><strong style="color:#FF3300"><?php echo $BL['be_fprivedit_err1'] ?></strong></p>
        </span>
	<?php } ?>

    <p>
        <b><?php echo $BL['be_fprivedit_filename'] ?></b>
        <input name="file_name" type="text" class="width400 v12" id="file_name" value="<?php echo html($file_name) ?>" size="40" maxlength="230">
    </p>
...
?>
	...
	<!--<tr><td colspan="2" valign="top"><img src="img/lines/line-bluelight.gif" alt="" width="<?php echo $phpwcms['LarguraInterna']; ?>" height="1"></td></tr>-->
	<!--<tr bgcolor="#F5F8F9"><td colspan="2" valign="top"><img src="img/leer.gif" alt="" width="1" height="6"></td></tr>-->

	<?php

	//Auswahlliste vordefinierte Keyw�rter
	$sql = "SELECT * FROM ".DB_PREPEND."phpwcms_filecat WHERE fcat_deleted=0 ORDER BY fcat_sort, fcat_name";
	if($result = mysqli_query($db, $sql) or die("aerror while browsing file categories for selecting keywords2")) {
		$k = "";
