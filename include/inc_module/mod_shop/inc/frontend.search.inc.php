	...
	function search() {
		...
		foreach($data as $value) {
			...
			if($s_count) {
				...
				$cat = explode(',',$value['shopprod_category']);
				...
				if($this->search_highlight) {
					...
					$this->search_results[$id]['link']	= shop_rel_url($alias_shop, $cat[0], $value['shopprod_id']);
				} else {
					$this->search_results[$id]["title"]	= $s_title;
					$this->search_results[$id]["text"]	= $s_text;
					$this->search_results[$id]['link']	= shop_rel_url($alias_shop, $cat[0], $value['shopprod_id']);
					/*rel_url(array('shop_cat' => $value['shopprod_category'], 'shop_detail' => $value['shopprod_id']), array('highlight', 'searchstart', 'searchwords'), $shop_url);*/
				}
