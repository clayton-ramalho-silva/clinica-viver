...
/**
 Envia o javascript selecionado para o rodapé, depois do Jquery
 */
function renderFooterJS($js) {
	...
	if(!$remote && (strpos($js, ';') !== false || strpos($js, '//') !== false || strpos($js, '/*') !== false)) {
		...
		// add the same section only once
		if(empty($GLOBALS['block']['custom_htmlfooter'][$key])) {

			$GLOBALS['block']['custom_htmlfooter'][$key] .= $js;
		...
		initJSLib();
			...
			// replace {TEMPLATE}
			$js		= str_replace('{TEMPLATE}', TEMPLATE_PATH, $js);
			$GLOBALS['block']['custom_htmlfooter'][md5($js)] = getJavaScriptSourceLink(html_specialchars($js));
...
function initVideoJs() {
	$GLOBALS['phpwcms']['video-js'] = empty($GLOBALS['phpwcms']['video-js']) ? PHPWCMS_HTTP_SCHEMA.'://vjs.zencdn.net/4.8/' : rtrim($GLOBALS['phpwcms']['video-js'], '/') . '/';
