...
<!--RSSFEED_START//-->[TITLE]<h3>{TITLE}</h3>[/TITLE][SUBTITLE]<h4>{SUBTITLE}</h4>[/SUBTITLE]<div class="rss-feed">{FEEDINFO}<ul>{ITEMS}</ul></div><!--RSSFEED_END//-->';
	...
	if( $rss_obj->data ) {
		...
		foreach($rss_obj->get_items() as $rssvalue) {
			...
			//---------------------- Alteração WebCis (18/09/2013) --------------------------------------------------
			// Limitar quantidade de palavras no comentário do RSS Feeds
			$words = explode(" ",$rssvalue->get_description());
			$limite = implode(" ",array_splice($words,0,$phpwcms['palavras']));
            $rss['items'][$c] = render_cnt_template($rss['items'][$c], 'DESCRIPTION', $limite.'...');
			//-------------------------------------------------------------------------------------------------------
