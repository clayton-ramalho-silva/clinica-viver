...
// Montagem da url da imagem do formulario ======
...
$CNT_TMP .= '<div class="bloco-formulario formaid'.$crow["acontent_id"].'"><section>';

if(empty($cnt_form['anchor_off'])) {
	$CNT_TMP .= '<a id="jumpForm'.$crow["acontent_id"].'"></a>';
}


$CNT_TMP .= headline($crow["acontent_title"], $crow["acontent_subtitle"], $template_default["article"]);
$CNT_TMP .= '  <div class="divider"></div>';
	...
	foreach($cnt_form["fields"] as $key => $value) {
        ...
        if($cnt_form["fields"][$key]['required']){
            ...
            if($value['type'] === 'checkbox'){
                $script_rules[] = '"'.$value['name'].'[]": {required: true, minlength: 1}';
                $script_msg[]   = '"'.$value['name'].'[]": "'.utf8_decode('Obrigatório').'"';
            } else if($value['type'] === 'email'){
                $script_rules[] = '"'.$value['name'].'": {required: true, email: true}';
                $script_msg[]   = '"'.$value['name'].'": {required: "", email: ""}';
            } else if($value['type'] === 'radio'){
                $script_rules[] = '"'.$value['name'].'": "required"';
                $script_msg[]   = '"'.$value['name'].'": "'.utf8_decode('Obrigatório').'"';
            } else {
                $script_rules[] = '"'.$value['name'].'": "required"';
                $script_msg[]   = '"'.$value['name'].'": ""';
            }
        }
		...
		switch($cnt_form["fields"][$key]['type']) {
			...
			case 'text'		:	/*
								...
								if($cnt_form["fields"][$key]['class']) {
									$form_field .= ' '.$cnt_form["fields"][$key]['class'].'';
								}
			...
			case 'checkbox':
								...
								if($checkbox_copy || count($form_value) == 1 || count($form_value) == 0 || !$form_value) {
									...
									foreach($form_value as $checkbox_value) {
										...
										$form_field .= '<span><input type="checkbox" name="'.$form_name.'[]" id="'.$form_name.$checkbox_counter.'" ';
										if(substr($checkbox_value, -8) != ' checked') {
											$form_field .= 'value="' . $checkbox_value . '" />';
										} else {
											$checkbox_value = str_replace(' checked', '', $checkbox_value);
										...
										$form_field .= $checkbox_label .'</label></span>';
										$checkbox_counter++;
									}
								}
			...
			case 'radio'	:	/*
								...
								$form_field 	.= '<span class="'.trim('form-radio'.$cnt_form["fields"][$key]['class']).'">';
								$checkbox_class  = '</span>';
								if($cnt_form["fields"][$key]['class']) {
			...
			case 'break'	:	/*
                                ...
                                //
                                $form_field .= '<input type="date" name="'.$form_name.'" id="'.$form_name.'" ';
                                $form_field .= 'value="'.html_specialchars($cnt_form["fields"][$key]['value']).'"';
                                if($cnt_form["fields"][$key]['size']) {
                                	$form_field .= ' size="'.$cnt_form["fields"][$key]['size'].'"';
                                }
                                ...
                                if($cnt_form["fields"][$key]['style']) {
                                	$form_field .= ' placeholder="'.$cnt_form["fields"][$key]['style'].'"';
                                }
                                $form_field .= ' />';
                                break;
			...
			case 'breaktext':	/*
                                ...
                                //
                                $form_field .= '<input type="tel" name="'.$form_name.'" id="'.$form_name.'" ';
                                $form_field .= 'value="'.html_specialchars($cnt_form["fields"][$key]['value']).'"';
                                if($cnt_form["fields"][$key]['size']) {
                                	$form_field .= ' size="'.$cnt_form["fields"][$key]['size'].'"';
                                }
...
if(!empty($POST_DO) && empty($POST_ERR)) {
	...
	$mail->Subject			= $cnt_form["subject"];
	$mail->Body 			= '<table width="750" border="0" cellspacing="1" cellpadding="0"><tr><td></td></tr></table>'.$cnt_form['template'];
...
if($form_cnt) {
    ...
    $recaptcha = json_decode(get_dados('dados_recaptcha'), true);

    $form_cnt = str_replace('###RESET###', '', $form_cnt);

    // Verifica se possui recaptcha do Google
    if($recaptcha['ativo'] === '1' && $cnt_form['google'] != 1) {

        $block["htmlhead"] .= '<script src="https://google.com/recaptcha/api.js"></script>';

        $site_key      = $recaptcha["site"];
        $div_recaptcha = '<div class="g-recaptcha" data-sitekey="'.$site_key.'"></div>';

        if(!empty($script_rules) && !empty($script_msg)){
            ...
            submitHandler: function(form, e) {
                ...
                if(grecaptcha.getResponse() == "") {
                    ...
                    e.preventDefault();
                    alert("'.utf8_decode("Por favor faça a verificação do recaptcha").'");

                } else {

                    $.post("include/inc_front/recaptcha.check.inc.php", {check: grecaptcha.getResponse()}, function(data, status){

                        if(data === "1"){
                            form.submit();
                        } else {
                            alert("'.utf8_decode("Erro na validação do recaptcha. Por favor tente novamente mais tarde").'");
                        }
            ...
            $block['custom_htmlfooter'][] = renderFooterJS('
            <script>
            $().ready(function() {
                $("#phpwcmsForm'.$crow["acontent_id"].'").on("submit", function(e){
                    if(grecaptcha.getResponse() == "") {
    ...
    if(!empty($script_rules) && !empty($script_msg)){
        ...
        // Scrip de Validação
        $rules = implode(",".LF.'                ', $script_rules);
        $msg   = implode(",".LF.'                ', $script_msg);

        $block['custom_htmlfooter'][] = renderFooterJS('js/jquery.validate.js');
        $block['custom_htmlfooter'][] = renderFooterJS('
        <script>
        $().ready(function() {
            $("#phpwcmsForm'.$crow["acontent_id"].'").validate({
                rules:{
                    '.$rules.'
                },
                messages:{
	...
	$form_cnt = str_replace('{ID}', $crow["acontent_id"], $form_cnt);
	...
	$CNT_TMP .= '</div>' . LF . $div_recaptcha. '<p> <button type="submit" id="enviar" name="enviar" value="Enviar">Enviar</button><!--<input type="submit" name="enviar" id="enviar" value="Enviar">--> </p> </form>' . LF . $cnt_form["class_close"];
}
