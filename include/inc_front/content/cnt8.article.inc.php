...
if((is_array($content['alink']['alink_id']) && count($content['alink']['alink_id'])) || (!empty($content['alink']['alink_type']) && ((is_array($content['alink']['alink_level']) && count($content['alink']['alink_level'])) || (isset($content['alink']['alink_category']) && count($content['alink']['alink_category']))))) {
	...
	$alink_sql  = "SELECT ar.*, UNIX_TIMESTAMP(ar.article_begin) AS article_date FROM ".DB_PREPEND."phpwcms_article ar ";
	...
	if(empty($content['alink']['alink_type'])) {
        ...
        $alink_sql .= " ORDER BY article_date DESC";
        $alink_sql .= $content['alink']['tags_group_by'];

        if($content['alink']['alink_paginate'] === 1){

            $count_sql = _dbQuery($alink_sql);
            $numRows   = count($count_sql);

            $rows     = $content['alink']['alink_itens'];
            $pagTotal = ceil($numRows / $rows); // Quantidade total de páginas

            // Faz os cálculos de páginas e limites com os dados informados
            $pagina   = get_pg();
            $pagAtual = ($pagina && is_numeric($pagina)) ? (int)$pagina : 1;
            $pagAtual = ($pagAtual > $pagTotal && $pagTotal > 0) ? $pagTotal : (($pagAtual < 1) ? 1 : $pagAtual);

            $offset   = ($pagAtual - 1) * $rows;

            $alink_sql .= ' LIMIT '.$offset.', '.$rows;

            // Links do paginate
            $link_paginate = ($content['alink']['alink_paginate'] === 1) ? links_paginate($pagTotal, 3) : '';
    ...
    //$content['alink']['alink_template_head']	= str_replace('{ID}', $content['alink']['alink_id'], $content['alink']['alink_template_head']);
    $content['alink']['alink_template_head']	= str_replace('{ID}', $crow["acontent_id"], $content['alink']['alink_template_head']);

    // echo '<pre>';
    // var_dump($content['alink']);
    // echo '</pre>';
    ...
    $content['alink']['alink_template_head']   = render_cnt_template($content['alink']['alink_template_head'],   'LINK',  $link_url);
    $content['alink']['alink_template_footer'] = render_cnt_template($content['alink']['alink_template_footer'], 'LINK',  $link_url);
    $content['alink']['alink_template_head']   = render_cnt_template($content['alink']['alink_template_head'],   'BOTAO', $texto_botao);
    $content['alink']['alink_template_footer'] = render_cnt_template($content['alink']['alink_template_footer'], 'BOTAO', $texto_botao);
		...
		foreach($content['alink']['alink_id'] as $key => $value) {
			...
			foreach($content['alink']['result'] as $row) {
				...
				if($value == $row['article_id'] && isset($content['struct'][ $row['article_cid'] ])) {
                    ...
                    $content['alink']['tr'][$key]	= str_replace('{ID}', $row['article_id'], $content['alink']['tr'][$key]);
					...
					// ---------------------------------- Altera��o  - Inser��o de classes -------------------------
					if($content['alink']['addclassemulti']) {
						if ($i % $content['alink']['numeromulti']==0){$content['alink']['tr'][$key] = str_replace('{MULTI}', ' '.$content['alink']['classemulti'], $content['alink']['tr'][$key]);} else {$content['alink']['tr'][$key] = str_replace('{MULTI}', '', $content['alink']['tr'][$key]);};
					} else {$content['alink']['tr'][$key] = str_replace('{MULTI}', '', $content['alink']['tr'][$key]);};

					if($content['alink']['addclasse']) {
						if ($i == $content['alink']['numeroclasse']){$content['alink']['tr'][$key] = str_replace('{CLASSE}',	' '.$content['alink']['addclasse_classe'], $content['alink']['tr'][$key]);} else {$content['alink']['tr'][$key] = str_replace('{CLASSE}',	'', $content['alink']['tr'][$key]);};
					} else {$content['alink']['tr'][$key] = str_replace('{CLASSE}', '', $content['alink']['tr'][$key]);};
					//-----------------------------------------------------------------------------------------------------

					$content['alink']['tr'][$key]	= render_cnt_template($content['alink']['tr'][$key], 'MENUTITLE', html_specialchars($row['article_menutitle']));
					if($row['article_id'] == $GLOBALS['content']['article_id']){
						$content['alink']['tr'][$key]	= render_cnt_template($content['alink']['tr'][$key], 'ATIVO', ' ativo');
					} else {
						$content['alink']['tr'][$key]	= render_cnt_template($content['alink']['tr'][$key], 'ATIVO', '');
					}
						...
						if($img_thumb_id) {
							...
							if(strpos($content['alink']['tr'][$key], 'cmsimage.php') !== false && $img_thumb_hash) {
								...
								if($thumb_image != false) {
									...
									$thumb_img .= ' class="lazyload" loading="lazy"/>';
					...
					if(strpos($content['alink']['tr'][$key], 'IMAGE_DETAIL') !== false && isset($row['article_image']['hash'])) {
						...
						if($row['article_image']['detail'] != false) {
							...
							$row['article_image']['img'] .= ' class="lazyload"/>';
					...
					$content['alink']['tr'][$key]	= render_cnt_template($content['alink']['tr'][$key], 'CAT', ' '.$content['struct'][ $row['article_cid'] ]['acat_class']);
    ...
    $content['alink'] = render_cnt_template($content['alink'], 'PAGINATE', $link_paginate);
