...
if($guestbook['visible']) {
	...
	if(!empty($guestbook['cookie']) && !empty($guestbook['time'])) {
		...
		if(!$guestbook['flooding']) {
			...
			if($guestbook['result'] = mysqli_query($db, $guestbook['sql'])) {
				if($guestbook['row'] = mysqli_fetch_row($guestbook['result'])) {
					if($guestbook['row'][0]+$guestbook['time'] >= time()) {
						$guestbook['flooding'] = 1;
						$guestbook['readform'] = 1;
					}
				}
				mysqli_free_result($guestbook['result']);
			}
		}
	}
	...
	if(isset($_POST['guestbook_email']) && !$guestbook['flooding']) {
		...
		if(!count($guestbook['error'])) {
			...
			mysqli_query($db, $guestbook['sql']);
			if($guestbook['new_entry_id'] = mysqli_insert_id($db)) {
				$guestbook['readform'] = 1;
	...
	if($guestbook['listing'] && $guestbook['listcount']) {
		...
		if($guestbook['result'] = mysqli_query($db, $guestbook['sql'].';')) {
			$guestbook['count'] = mysqli_num_rows($guestbook['result']);
			mysqli_free_result($guestbook['result']);
		}
		...
		if($guestbook['pagecount'] > 1 || $guestbook['archivedate']) {
			...
			if( ! ( strpos($guestbook['nav'],'{ARCHIVE')===false ) ) {
				...
				if($guestbook['result'] = mysqli_query($db, $guestbook['asql'])) {

					while($guestbook['row'] = mysqli_fetch_row($guestbook['result'])) {
	...
	if($guestbook['result'] = mysqli_query($db, $guestbook['sql'])) {
		...
		while($guestbook['row'] = mysqli_fetch_assoc($guestbook['result'])) {
