...
if($image['template']) {
	...
	if(is_array($image['images']) && ($image['count'] = count($image['images']))) {
		...
		foreach($image['images'] as $key => $value) {
			...
			$list_img_temp .= $thumb_image[3].' alt="'.$caption[1].'"'.$caption[3].' class="'.$template_default['classes']['image-thumb'].'" />';
			$img_a			= '';
			$lightbox_capt  = '';
                        $texto_botao    = ($value['botao']) ? $value['botao'] : 'Saiba Mais';
                        $link_url       = ($value['tipo'] === '1') ? $value['url'] : get_alias($value['pagina']);
			...
			// ---------------------------------- Alteração WebCis - Inserção de Múltiplo -------------------------
			if($image['addclassemulti']) {
				if ($i % $image['numeromulti']==0){$img_a = str_replace('{MULTI}', ' '.$image['classemulti'], $img_a);} else {$img_a = str_replace('{MULTI}',	'', $img_a);};
			} else {$img_a = str_replace('{MULTI}', '', $img_a);};
			if($image['addclasse']) {
				if ($i == $image['numeroclasse']){$img_a = str_replace('{CLASSE}', ' '.$image['addclasse_classe'], $img_a);} else {$img_a = str_replace('{CLASSE}',	'', $img_a);};
			} else {$img_a = str_replace('{CLASSE}', '', $img_a);};
			//-----------------------------------------------------------------------------------------------------
			...
			$img_a = render_cnt_template($img_a, 'URL', $link_url);
                        $img_a = render_cnt_template($img_a, 'URL_TARGET', $value['url_target']);

                        $img_a = render_cnt_template($img_a, 'BOTAO', $texto_botao);
	...
	$image['template']  = render_cnt_template($image['template'], 'EFFECT_3', ($image['fx3'] ? '<!-- FX 3 -->' : '') );
        ...
        $links_url       = ($crow["acontent_all_link"]['tipo'] === '1') ? $crow["acontent_all_link"]['link'] : get_alias($crow["acontent_all_link"]['pag']);
        $ncolunas       = ($crow["acontent_all_link"]['cimage_colunas']);

        $image["template"] = render_cnt_template($image["template"], 'LINK', $links_url);
        $image["template"] = render_cnt_template($image["template"], 'BOTAO', $texto_botao);
        $image["template"] = render_cnt_template($image["template"], 'COLUNAS', $ncolunas);
        
        $crow["campos_extra"] = unserialize($crow["campos_extra"]);
        $clone       = ($content['campos_extra']['campo_clonar']);

//        	$image['template']  = str_replace('{CLONE}', $crow["campos_extra"]['campo_clonar'], $image['template']);

        $image["template"] = render_cnt_template($image["template"], 'CLONE', $crow["campos_extra"]['campo_clonar']);
