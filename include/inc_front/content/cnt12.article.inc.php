...
$CNT_TMP .='<div class="newsletter"><section>';
...
if(!$content["newsletter"]["change_text"] || !$content["newsletter"]["reg_text"]) {
	$temp_mailtext  = 'O e-mail informado já foi cadastrado anteriormente em nosso site.'."\n";
	$temp_mailtext .= 'Seu e-mail: {NEWSLETTER_EMAIL}.'."\n\n";
	$temp_mailtext .= 'Para confirmar o seu cadastro clique no link abaixo.'."\n";
	$temp_mailtext .= '{NEWSLETTER_VERIFY}'."\n\n";
	$temp_mailtext .= 'Para apagar seu cadastro a partir de nossa base de dados clique no link abaixo:'."\n";
	$temp_mailtext .= '{NEWSLETTER_DELETE}'."\n\n";
	$temp_mailtext .= 'Atenciosamente,'."\n";
...
if(isset($_POST["newsletter_send"]) && intval($_POST["newsletter_send"])) {
	...
	if(is_valid_email($content["newsletter"]["email_address"])) {
		...
		if(empty($content['newsletter']['subject']['tag'])) {
			...
			$content['newsletter']['subject'] = $phpwcms['SMTP_FROM_NAME'];
		} else {
			$content["newsletter"]["mailtext"]	= $content['newsletter']['subject']['new'];
...
if($content["newsletter"]["success"]) {
	...
	$CNT_TMP .= '<div id="newsletter"> <i class="icon-email"></i>' ;
	$CNT_TMP .= 	
							($content["newsletter"]["success_text"]) ? '<style>#newsletter i{display:none;}</style><div class="obrigado">'.nl2br($content["newsletter"]["success_text"]).'</div>' : '<div class="obrigado">'."Obrigado, seu e-mail foi cadastrado com sucesso em nossos sistemas.".'</div>'
	;
	$CNT_TMP .= '</div>' ;
} else {
	...
	switch($content["newsletter"]["pos"]) {
		case 0:
			$CNT_TMP .= '<div id="newsletter"> ' ;
        	if($content["newsletter"]["email_address_error"]) {
               
                $CNT_TMP .= "<div class=\"texto-erro\">".$template_default["article"]["newsletter_error"]."</div>";
        	}
        	$CNT_TMP .= ($content["newsletter"]["text"]) ? "".nl2br(div_class($content["newsletter"]["text"],$template_default["article"]["text_class"])) : "";
        	$CNT_TMP .= '<form action="'.FE_CURRENT_URL.'#newsletter" method="post" id="newsletterSubscribeForm">'.LF;
        	
        	
        
        	$CNT_TMP .= $label_pos_tr;
        	$CNT_TMP .= "<input name=\"newsletter_email\" type=\"text\" class=\"inputNewsletter\" placeholder=\"Informe seu e-mail".$content["newsletter"]["label_email"]."\" ";
        	$CNT_TMP .= "value=\"".$content["newsletter"]["email_address"]."\" />\n";
       
        	$CNT_TMP .= "<input name=\"submit\" type=\"submit\" class=\"formButton\" value=\"";
        	$CNT_TMP .= (($content["newsletter"]["button_text"]) ? $content["newsletter"]["button_text"] : "Cadastrar")."\" />";
		...
		$CNT_TMP .= ($content["newsletter"]["text"]) ? "<br />".nl2br(div_class($content["newsletter"]["text"],$template_default["article"]["text_class"])) : "";
			...
			if($content["newsletter"]["email_address_error"]) {
				...
				$CNT_TMP .= "<td class=\"formError\" id=\"newsletter\">".$template_default["article"]["newsletter_error"]."</td>\n</tr>\n";
			}
			$CNT_TMP .= "<tr>\n";
			$CNT_TMP .= $label_pos_tr;
			...
			$CNT_TMP .= (($content["newsletter"]["button_text"]) ? $content["newsletter"]["button_text"] : "")."\" />";
			$CNT_TMP .= '<input name="newsletter_send" type="hidden" value="1" />';
			$CNT_TMP .= "</td>\n</tr>\n</table></form>";
		break;
