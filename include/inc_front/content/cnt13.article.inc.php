...
if(!empty($_POST["search_input_field"]) || !empty($_GET['searchwords'])) {
	...
	if(count($content['highlight'])) {
		...
		$sql .= "ar.article_aktiv=1 AND ar.article_deleted=0 AND ar.article_nosearch!=1 AND ";
		...
		if($sresult = mysqli_query($db, $sql)) {
			...
			while($srow = mysqli_fetch_assoc($sresult)) {
				...
				if($scresult = mysqli_query($db, $csql)) {
					while($scrow = mysqli_fetch_row($scresult)) {
			...
			if($GLOBALS['_search_navi'][0] === '123') {
				...
				for($_search_page_i = 1; $_search_page_i <= $_search_max_pages; $_search_page_i++) {
					...
					if($_search_current_page == $_search_page_i) {
						$_search_navi_x[$_search_page_i] .= '<span>' .$_search_page_i. '</span>';
					} else {
