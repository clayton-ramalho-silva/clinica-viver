...
function showSelectedContent($param='', $cpsql=null, $listmode=false) {
	...
	// Content Part mode CP, CPA, CPAD, CPS, CPAS, CPASD
		...
		foreach($cp as $value) {
			...
			if(!empty($sql) && ($cresult = mysqli_query($db, $sql))) {
...
function getContentPartSpacer($space_before=0, $space_after=0) {
	...
	if(!$space_before && !$space_after) {
		...
		return array('before' => '<div style="margin:' .$space_before. 'px 0 ' .$space_after. 'px 0; padding:0;" class="spaceBeforeCP spaceAfterCP">',	'after'  => '</div>');

	} elseif($space_before && !$space_after) {
		return array('before' => '<div style="margin:' .$space_before. 'px 0 0 0;padding:0;clear:both;" class="spaceBeforeCP"></div>', 'after'  => '');

	} else {
		return array('before' => '', 'after'  => '<div style="margin:0 0 ' .$space_after. 'px 0;padding:0;clear:both;" class="spaceAfterCP"></div>');
...
function getContentPartAlias($crow) {
	...
	if(!empty($alias['alias_ID'])) {
		...
		$sql_alias .= "LIMIT 1";
		if($alias_result = mysqli_query($db, $sql_alias)) {
