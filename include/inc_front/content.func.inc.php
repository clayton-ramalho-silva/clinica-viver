...
$content['CB']['BANNER']	= ''; // {BANNER}
$content['CB']['SERVICOS']	= ''; // {SERVICOS}
$content['CB']['PRODUTOS']	= ''; // {PRODUTOS}
$content['CB']['MAPA']		= ''; // {MAPA}
$content['CB']['CLIENTES']	= ''; // {CLIENTES}
...
if(!$aktion[4]) {
	...
	if($content['404error']['status'] === false && ($content["article_list_count"] || $content['struct'][ $content['cat_id'] ]['acat_topcount'] == -1)) {
		...
		if($content["article_list_count"] == 1 || $content['struct'][ $content['cat_id'] ]['acat_topcount'] == -1) {
			...
			// Gera um título H1 nas páginas com paginate
			$content["main"] .= '<div class="titulo"><h1><span>'.$content['struct'][ $content['cat_id'] ]['acat_name'].'</span></h1></div><div class="lista-paginas p'.$content["cat_id"].'"><div class="grid">'.list_articles_summary().'</div>';
			// -----------------------------------------------------------------------------
		}
...
if($aktion[2] == 0) {
	...
	switch($pagelayout["layout_render"]) {
		...
		case 1:	//create the page layout based on DIV (layer)
                ...
                // Alteração - Analytics ---------------------------------------
                // $dados = get_dados('dados_analytics');
				...
				// --- Alteração - Permite que o template do site seja chamando de arquivos externos ----
				$topo 	  = $block["headertext"];
				$conteudo = $block["maintext"];
				$rodape	  = $block["footertext"];

				$block["headertext"] = '';
				$block["maintext"] = '';
				$block["footertext"] = '';

				foreach($topo as $value){
					$block["headertext"] .= file_get_contents(PHPWCMS_ROOT.'/template/themes/'.$value, TRUE );
				}
				//$block["headertext"] .= include(PHPWCMS_ROOT.'/template/inc_script/frontend_render/rt_nav_horiz_drop_down.html');

				foreach($conteudo as $value){
					$block["maintext"] 	 .= file_get_contents(PHPWCMS_ROOT.'/template/themes/'.$value, TRUE );
				}

				foreach($rodape as $value){
					$block["footertext"] .= file_get_contents(PHPWCMS_ROOT.'/template/themes/'.$value, TRUE );
				}
				// ------------------------------------- Fim da Alteração --------------------------------------


				//header DIV
				//if($block["headertext"] || $pagelayout['layout_header_height']) { //Original
					$content["all"] .= '<header id="headerBlock">'.$block["headertext"]."</header>\n";
				...
				//main block
				$content["all"] .= '<main id="mainBlock">'.$block["maintext"]."</main>\n";
				//footer DIV
				//if($block["footertext"] || $pagelayout['layout_footer_height']) { //Original
					$content["all"] .= '<footer id="footerBlock">'.$block["footertext"]."</footer>\n";
...
$content['all'] = preg_replace_callback('/{HEAD:(.*?)}/s', 'renderHeadJS', $content['all']);
$content['all'] = preg_replace_callback('/{CSS:(.*?)}/s', 'renderHeadCSS', $content['all']);
$content['all'] = preg_replace_callback('/\[JS\](.*?)\[\/JS\]/s', 'renderFooterJS', $content['all']);
...
// Função para adicionar scripts antes do fechamento da tag BODY
if(count($block['custom_htmlfooter'])) {

foreach($block['custom_htmlfooter'] as $key => $value) {

$value = trim($value);

$block['htmlfooter'] .= $value.LF;
