	...
	if($result = mysqli_query($db, $sql)) {
		while($row = mysqli_fetch_assoc($result)) {
			$data[$row["acat_id"]] = array(
                ...
                "acat_img_width"	=> $row['acat_img_width'],
                "acat_img_height"	=> $row['acat_img_height'],
                "acat_img_crop"	    => $row['acat_img_crop'],
		...
		if($GLOBALS['paginate_temp']['navi']) {
			...
			if($navi[0] == '123') {
				...
				for($i = 1; $i <= $max_pages; $i++) {
					...
					$navi['navi'] .= ($i == $page_current) ? '<span>'.$i.'</span>' : '<a href="' . rel_url( array('listpage'=>$i) ) . '">'.$i.'</a>';
				...
				for($i = 0; $i < $max_pages; $i++) {
					...
					$i_page 	= $i+1;
					$navi['navi'] .= ($i_page == $page_current) ? '<span>'.$i_entry.'</span>' : '<a href="' . rel_url( array('listpage'=>$i_page) ) . '">'.$i_entry.'</a>';
	...
	foreach($content["articles"] as $article) {
		...
		if($temp_counter < $temp_topcount) {
			...
			if($tmpllist[ $article["article_image"]['tmpllist'] ]) {
                ...
                // echo $content['struct'][ $content['cat_id'] ][ 'acat_img_crop' ].'|<br>';
                // var_dump($content['struct'][ $content['cat_id'] ]);
				...
				$tmpl = str_replace(
					...
					array(
						...
						$article["article_image"]["list_hash"],
                        $img_thumb_ext,
                        $content['struct'][ $content['cat_id'] ][ 'acat_img_width' ],
                        $content['struct'][ $content['cat_id'] ][ 'acat_img_height' ],
                        $content['struct'][ $content['cat_id'] ][ 'acat_img_crop' ] ? $content['struct'][ $content['cat_id'] ][ 'acat_img_crop' ] : '0',
					),
					$tmpl
				);
...
function html_parser($string) {
	...
	$replace[5] 	= '<a href="#" target="_top" title="Voltar" onclick="history.back();return false;" class="'.$GLOBALS['template_default']['classes']['link-back'].'">$1</a>';
...
function _checkShopLogin($user='', $pass='') {
	if(empty($user) || empty($pass)) return false;
	// check against database
	$sql  = 'SELECT * FROM '.DB_PREPEND.'phpwcms_userdetail WHERE ';
	$sql .= "detail_login='".aporeplace($user)."' AND ";
	...
	return (isset($result[0]) && is_array($result)) ? $result[0] : 'bb';
}
