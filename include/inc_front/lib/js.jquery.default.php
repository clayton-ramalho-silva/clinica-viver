...
function initSlimbox() {
	initJSLib();
	$GLOBALS['block']['custom_htmlfooter']['lightbox.css'] = '  <link href="'.TEMPLATE_PATH.'lib/slimbox/slimbox.css" rel="stylesheet" type="text/css" media="screen" />';
	$GLOBALS['block']['custom_htmlfooter']['slimbox.js'] = getJavaScriptSourceLink(TEMPLATE_PATH.'lib/slimbox/slimbox.jquery.js');
}
...
function initJSPlugin($plugin='') {
	if(empty($GLOBALS['block']['custom_htmlhead'][$plugin])) {
		$GLOBALS['block']['custom_htmlhead'][$plugin] = $plugin;
	}
	return TRUE;
}
