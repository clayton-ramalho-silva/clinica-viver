...
if(isset($_GET["aktiv"])) {
	...
	$result = mysqli_query($db, $sql) or die ("error while changing ACTIVE status");
}
...
if(isset($_GET["public"])) {
	...
	$result = mysqli_query($db, $sql) or die ("error while changing PUBLIC status");
}
...
if(isset($_GET["delete"])) {
	...
	if($wert == 9) {
		...
		$result = mysqli_query($db, $sql) or die ("error while deleted directory");
	}
}
...
if(isset($_GET["trash"])) {
	...
	if($wert == 1 || $wert == 9 || $wert == 0) {
		...
		$sql .= " AND f_uid=".intval($_SESSION["wcs_user_id"]);
		$result = mysqli_query($db, $sql) or die ("error while moving file to trash");
	}
}
...
if(isset($_GET["paste"])) {
	...
	$result = mysqli_query($db, $sql) or die ("error while moving file to other directory");
}
...
if($_SESSION["wcs_user_admin"] == 1) { //Wenn Benutzer Admin-Rechte hat
	...
	if(isset($_GET['movedeletedfiles']) && intval($_GET['movedeletedfiles']) == $_SESSION["wcs_user_id"]) {
		...
		if($result = mysqli_query($db, $sql) or die ("error while retrieving list of deleted files")) {
