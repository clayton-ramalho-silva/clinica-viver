	...
	switch(intval($do[0])) {
		...
		case 1: //Aktiv/Inaktiv File Category
				...
				if($do[1]) {
					...
					mysqli_query($db, $sql) or die("error while changing file category status");
				}
				break;
		...
		case 2: //Aktiv/Inaktiv File Key
				...
				if($do[1]) {
					...
					mysqli_query($db, $sql) or die("error while changing file key status");
				}
				break;
		...
		case 8: //Löschen der File Category
				...
				if($do[1]) {
					...
					mysqli_query($db, $sql) or die("error while deleting file category");
				}
				break;
		...
		case 9: //Löschen des File Keys
				...
				if($do[1] && $do[2]) {
					...
					mysqli_query($db, $sql) or die("error while deleting file key");
				}
				break;
