...
if($_SESSION["wcs_user_admin"] == 1) { //Wenn Benutzer Admin-Rechte hat
	...
	if(isset($_POST["acat_id"]) && $_POST["acat_id"] === 'index') {
        ...
        $sql .= "\$indexpage['acat_menu']		= '".	str_replace("''", "\\'", getpostvar($_POST["acat_menu"]))."';\n";
	...
	if(isset($_POST["acat_new"]) && intval($_POST["acat_new"]) == 1 && intval($_POST["acat_id"]) == 0 && $_POST["acat_id"] != 'index') {
		if(trim($_POST["acat_name"])) {
			...
			if($result = mysqli_query($db, $sql) or die("MySQL Error: ".mysqli_error($db))) {
				$ref .= "&cat=".mysqli_insert_id($db);
			}
		}
	}
	...
	if(isset($_POST["acat_new"]) && isset($_POST["acat_id"]) && intval($_POST["acat_new"]) == 0 && intval($_POST["acat_id"])) {
		if(trim($_POST["acat_name"])) {
			...
			$sql =	"UPDATE ".DB_PREPEND."phpwcms_articlecat SET ".
                ...
                "acat_img_width="._dbEscape($_POST["acat_img_width"]).','.
                "acat_img_height="._dbEscape($_POST["acat_img_height"]).','.
                "acat_img_crop=".(isset($_POST["acat_img_crop"]) ? 1 : 0).','.
			...
			mysqli_query($db, $sql) or die(_report_error('DB', $sql));
		}
	}
...
switch(intval($do[0])) {
	...
	if($do[1]) { // && $do[2] = 0 f�r Root
	$sql =  "UPDATE ".DB_PREPEND."phpwcms_articlecat SET acat_struct=".$do[2].", acat_sort=".$do[3]." WHERE acat_id=".$do[1];
	mysqli_query($db, $sql) or die("error while updating structure level");
	}
	break;

	case 2:	//Sortierung �ndern
	...
	if($do[1] && $do[2]>=10 && $do[3] && $do[4]>=10) {
		...
		mysqli_query($db, $sql) or die("error while updating sorting ID1");
		$sql =  "UPDATE ".DB_PREPEND."phpwcms_articlecat SET acat_sort=".$do[4]." WHERE acat_id=".$do[3];
		mysqli_query($db, $sql) or die("error while updating sorting ID1");
	}
	break;

	case 3:	//Artikel einf�gen in
	...
	if($do[1]) { // && $do[2] = 0 f�r Root
		...
		mysqli_query($db, $sql) or die("error while updating article level");
	}
	break;

	case 4:	//Sortierung Artikel �ndern
	...
	if($do[1] && $do[2]>=10 && $do[3] && $do[4]>=10) {
		...
		mysqli_query($db, $sql) or die("error while updating sorting article ID1");
...
//19-11-2004  Fernando Batista start-----------------------------------------------------------------------------------------------------------
	...
	$result = mysqli_query($db, $sql) or die("error while updating Article Content");
	...
	mysqli_query($db, $sql) or die("error while updating Article Content");
	...
	$sql  = "SELECT * FROM ".DB_PREPEND."phpwcms_articlecontent WHERE acontent_id=".$do[1];
			if($result = mysqli_query($db, $sql) or die("error sql")) {
				  $row = mysqli_fetch_assoc($result);
					...
					$result = mysqli_query($db, $sql2) or die("error while copy article content <br>error while connecting to database: <br><pre>".$sql2."</pre>");
	...
	if($do[1]) {
		...
		if(count($article_del)) {
			...
			foreach($article_del as $value) {
				...
				mysqli_query($db, $sql) or die("error while deleting cached article ID:".$value);
			...
			if($a_del) {
				...
				mysqli_query($db, $sql) or die("error while deleting articles while deleting structures");
			}
		}
		...
		if(count($struct_del)) {
			...
			foreach($struct_del as $value) {
				...
				mysqli_query($db, $sql) or die("error while deleting cached category ID:".$value);
			...
			if($s_del) {
				...
				mysqli_query($db, $sql) or die("error while deleting structures");
			}
		}
...
function copy_article_to_level($do, $dbcon) {
	...
	if($result = mysqli_query($dbcon, $sql) or die("error while connecting to database: <pre>".$sql."</pre>")) {
		...
		if($result = mysqli_query($dbcon, $sql) or die("error while copy article <br>error while connecting to database: <pre>".$sql."</pre>")) {

			$article_insert_id = mysqli_insert_id($dbcon);

			$sql1  = "SELECT * FROM ".DB_PREPEND."phpwcms_articlecontent WHERE acontent_aid=".$do[1];
			if($result1 = mysqli_query($dbcon, $sql1) or die("error sql")) {
				while ($row1 = mysqli_fetch_assoc($result1)) {
					...
					$sql2 =  "INSERT INTO ".DB_PREPEND."phpwcms_articlecontent (".$key1s.") VALUES (".$value1s.")";
					$result = mysqli_query($dbcon, $sql2) or die("error while copy article content <br>error while connecting to database: <pre>".$sql2."</pre>");
				}
...
function copy_level_to_level($do, $dbcon) {
	...
	if($result = mysqli_query($dbcon, $sql) or die("error while connecting to database: <br><pre>".$sql."</pre>")) {
		if($row = mysqli_fetch_assoc($result)) {
		...
		$acat_insert_id = mysqli_insert_id($dbcon);
	}
