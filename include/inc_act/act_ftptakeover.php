	...
	foreach($ftp["mark"] as $key => $value) {
		...
		if(is_file($file_path)) {
			...
			if(isset($result['INSERT_ID'])) {
				...
				$file_error["upload"] = 'MySQL Error while insert to DB: '.mysqli_error();
...
if(empty($file_error["upload"]) && empty($ftp["error"])) {
	...
	echo "<p class='v10'><a href=\"".$ref."\" style=\"font-weight: bold;\">Clique aqui para voltar</a> (if no automatic redirect)</p>\n";
