	...
	if(count($values)) {
		switch(intval($values[0])) {
			case 9: //delete article content part
					...
					mysqli_query($db, $sql) or die("error while deleting content part");
					break;
			case 1: //delete article
					...
					mysqli_query($db, $sql) or die("error while deleting article");
					$ref .= '&p=&s=&id=';
					break;
			case 2: //make content visible/invisible
					$sql = "UPDATE ".DB_PREPEND."phpwcms_articlecontent SET acontent_visible=".intval($values[3]).
						   ...
						   " AND acontent_aid=".intval($values[1]).
						   " AND acontent_id=".intval($values[2]).";";
					mysqli_query($db, $sql) or die("error while changing content visible/invisible");
					break;
			case 3: //make article visible/invisible
					...
					mysqli_query($db, $sql) or die("error while changing article visible/invisible");
					break;
...
if(isset($_GET["sort"])) {
	...
	$sql2 = "UPDATE ".DB_PREPEND."phpwcms_articlecontent SET acontent_sorting=".$sort2.
		    " WHERE (acontent_uid=".intval($_SESSION["wcs_user_id"])." OR ".intval($_SESSION["wcs_user_admin"]).")".
		    " AND acontent_id=".$id2;
	mysqli_query($db, $sql1) or die("error while changing content part's sorting");
	mysqli_query($db, $sql2) or die("error while changing content part's sorting");
}
