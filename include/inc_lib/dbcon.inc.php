...
if($phpwcms["db_pers"] == 1) {
	$db = @mysqli_connect($phpwcms["db_host"], $phpwcms["db_user"], $phpwcms["db_pass"]) or ($is_mysql_error = true);
} else {
	$db = @mysqli_connect($phpwcms["db_host"], $phpwcms["db_user"], $phpwcms["db_pass"]) or ($is_mysql_error = true);
}
...
function _setConfig($key, $value=NULL, $group='', $status=1) {
	...
	foreach($key as $k => $value) {
		...
		if ( ! _dbInsertOrUpdate('phpwcms_sysvalue', $data) ) {
			$mysql_error = trim( @mysqli_error($db) );
...
function _getConfig($key, $set_global='phpwcms') {
	...
	if(is_array($key) && count($key)) {
		...
		foreach($key as $value) {
			...
			$sql = 'SELECT * FROM '.DB_PREPEND."phpwcms_sysvalue WHERE sysvalue_status=1 AND sysvalue_key='".mysqli_real_escape_string($db, $value)."'";
			$row = _dbQuery($sql);
			if(isset($row[0])) {
...
// ALT: Busca a lista de p�ginas do site (parte de conte�do Banner)
function get_pages($page=""){

    $sql = 'SELECT article_id, article_title
            FROM '.DB_PREPEND.'phpwcms_article
            WHERE article_deleted != 9';
    $res = _dbQuery($sql);

    $lista = '';
    foreach($res as $value){

        $check  = ($value['article_id'] === $page) ? ' selected' : '';
        $lista .= '<option value="'.$value['article_id'].'"'.$check.'>'.$value['article_title'].'</option>';
...
// ALT: Busca o alias da p�gina com base no AID
function get_alias($aid){

    $sql = 'SELECT article_alias
            FROM '.DB_PREPEND.'phpwcms_article
            WHERE article_id = '.$aid;
    $res = _dbQuery($sql);

    return $res[0]['article_alias'].'.html';
