...
if($acat_id) { //Anzeige der gew�hlten Artikel Kategorie
	...
	if($result_acat = mysqli_query($db, $sql) or die("error while reading article category infos")) {
		if($row_acat = mysqli_fetch_assoc($result_acat)) {
            ...
            $acat_img_width		= $row_acat["acat_img_width"];
            $acat_img_height	= $row_acat["acat_img_height"];
            $acat_img_crop	    = $row_acat["acat_img_crop"];
			$acat_disable301	= $row_acat['acat_disable301'];
			$acat_opengraph		= $row_acat['acat_opengraph'];
		}

		mysqli_free_result($result_acat);
	}
} else {
