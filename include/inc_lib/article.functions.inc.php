...
function struct_select_menu($counter=0, $struct_id=0, $selected_id=0, $return='option') {
	...
	if($result = mysqli_query($GLOBALS['db'], $sql) or die ("error while building struct select menu (ID:".$struct_id)) {
		$sx=0;
...
function struct_select_list($counter=0, $struct_id=0, & $selected_id) {
	...
	if($result = mysqli_query($db, $sql) or die ("error while building struct select menu (ID:".$struct_id)) {
		$sx=0;
		while($row = mysqli_fetch_assoc($result)) {
			$struct[$sx] = $row;
			$sx++;
