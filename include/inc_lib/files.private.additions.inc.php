...
if(isset($_GET["all"])) { //Übernehmen der Aufklappwerte aus Session-Variable
	...
	if($_GET["all"] == "open") { //alle aufklappen
		...
		if($result = mysqli_query($db, $sql) or die ("error while open all directories")) {
	...
	mysqli_query($db, "UPDATE ".DB_PREPEND."phpwcms_user SET usr_var_privatefile="._dbEscape(serialize($_SESSION["klapp"]))." WHERE usr_id=".$_SESSION["wcs_user_id"]);
