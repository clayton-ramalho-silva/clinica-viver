...
function list_profession($c){
	...
	$sql = mysqli_query($db, "SELECT prof_name FROM ".DB_PREPEND."phpwcms_profession ORDER BY prof_name");
...
function which_folder_active($ist, $soll, $ac="#666666", $nc="#333333", $nclass="msgreiter") {
	if($ist == $soll) {
		echo "bgcolor='".$ac."' class='".$nclass."'";
	} else {
		echo "bgcolor='".$nc."' class='".$nclass."' ";
		echo "onMouseOver=\"bgColor='#999999'\" onMouseOut=\"bgColor='".$nc."'\"";
	}
}
...
function online_users($dbcon, $spacer="<br />", $wrap="<span class=\"useronline\">|<span>") {
	...
	$wrap = explode("|", $wrap);
	$x=0; $xo="";
	if($o = mysqli_query($db, "SELECT logged_user FROM ".DB_PREPEND."phpwcms_userlog WHERE logged_in=1", $dbcon)) {
		while($uo = mysqli_fetch_row($o)) {
			$xo .= ($x) ? $spacer : "";
			$xo .= html($uo[0]);
			$x++;
		}
...
function get_list_of_file_keywords() {
    ...
    global $db;
	if($result = mysqli_query($db, "SELECT * FROM ".DB_PREPEND."phpwcms_filekey")) {
...
function getFormTrackingValue() {
	...
	if(!empty($GLOBALS['phpwcms']["form_tracking"])) {
		...
		if($entry_created = mysqli_query($GLOBALS['db'], $sql)) {
			$entry_id = mysqli_insert_id($GLOBALS['db']);
		}
	}
