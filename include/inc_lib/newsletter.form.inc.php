...
if(!empty($_GET["del"]) && intval($_GET["del"]) == $newsletter["newsletter_id"]) {
	...
	mysqli_query($db, $sql) or die("error while deleting newsletter");
...
if(isset($_POST["newsletter_id"])) {
	...
	if(!isset($newsletter['error'])) {
		...
		mysql_query($db, $sql) or die("error while updating or inserting newsletter datas");
		if(!$newsletter["newsletter_id"]) $newsletter["newsletter_id"] = mysqli_insert_id($db);
